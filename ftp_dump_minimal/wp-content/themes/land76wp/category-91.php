<?php
/*
Template Name: Категория Ливневая канализация
*/
?>
...
<?php
$cat91_region = function_exists('land76wp_get_current_livnevka_region') ? land76wp_get_current_livnevka_region() : null;
$cat87_term_context = $cat91_region ? 'category_91' : 'category_' . get_queried_object_id();
...
$cat91_region_seo_content = '';
if ($cat91_region) {
    $cat87_hero_title = 'Ливневая канализация в ' . $cat91_region['locative'] . ' под ключ';
    $cat87_hero_subtitle = $cat91_region['lead'];
    $cat87_hero_btn_primary_text = 'Рассчитать ливневку';
    ...
    $cat87_prices_title = 'Стоимость ливневой канализации в ' . $cat91_region['locative'];
    $cat87_faq_title = 'Вопросы по ливневой канализации в ' . $cat91_region['locative'];
    $cat87_faq_items = array(
        array(
            'question' => 'Чем ливневая канализация отличается от дренажа?',
            'answer' => 'Ливневка собирает дождевую и талую воду с крыши, дорожек и площадок. Дренаж работает с водой в грунте. На сложных участках системы часто связывают, но задачи у них разные.',
        ),
        array(
            'question' => 'Можно ли сделать ливневку на готовом участке?',
            'answer' => 'Да. Мы учитываем существующие дорожки, плитку, газон, водостоки, посадки и точки сброса, чтобы аккуратно проложить трассы и не сломать благоустройство.',
        ),
        array(
            'question' => 'Что входит в монтаж ливневой канализации?',
            'answer' => 'Обычно это расчет схемы, дождеприемники, лотки, трубы, колодцы, пескоуловители, уклоны, точка сброса и проверка прохождения воды.',
        ),
    );
    $cat91_region_seo_content = !empty($cat91_region['content']) ? $cat91_region['content'] : '';
}
...
if (empty($cat87_price_rows) || !is_array($cat87_price_rows)) {
    $cat87_price_rows = array(
        array('service' => 'Монтаж ливневой канализации', 'price' => 'от 2 500 ₽ за м.п.', 'term' => '3-7 дней'),
        array('service' => 'Линейный водоотвод', 'price' => 'от 2 200 ₽ за м.п.', 'term' => '1-4 дня'),
        array('service' => 'Дождеприемники и колодцы', 'price' => 'по расчету', 'term' => '1-3 дня'),
    );
}

if (empty($cat87_estimate_items) || !is_array($cat87_estimate_items)) {
    $cat87_estimate_items = array(
        array('item' => 'Схема приема и отвода воды - по расчету'),
        array('item' => 'Дождеприемники, лотки и трубы - по расчету'),
        array('item' => 'Колодцы, пескоуловители и точка сброса - по расчету'),
        array('item' => 'Земляные работы и монтаж - по расчету'),
    );
}
...
if (empty($cat87_faq_items) || !is_array($cat87_faq_items)) {
    $cat87_faq_items = array(
        array(
            'question' => 'Сколько стоит ливневая канализация?',
            'answer' => 'Стоимость зависит от площади водосбора, количества водостоков, длины трасс, лотков, колодцев и точки сброса. Точную цену считаем после осмотра и схемы.',
        ),
        array(
            'question' => 'Можно ли соединять ливневку с дренажом?',
            'answer' => 'Не всегда. Ливневая вода с крыши идет большим потоком и может перегрузить дренаж. Схему подключения подбираем по месту.',
        ),
        array(
            'question' => 'Сколько времени занимает монтаж?',
            'answer' => 'Небольшой водоотвод можно сделать за несколько дней. Полноценная система с водостоками, трубами, колодцами и лотками обычно занимает от 3 до 7 дней.',
        ),
    );
}
?>
    <!-- 1. Hero блок с полной структурой из header-page.php -->
      <section class="hero">
        ...
        <div class="hero__content wrapper">
          <h1 class="hero__title" data-aos="fade-right" data-aos-duration="800"><?php echo esc_html($cat87_hero_title ? $cat87_hero_title : 'Ливневая канализация под ключ'); ?></h1>
          <p class="hero__subtitle" data-aos="fade-up" data-aos-duration="900"><?php echo esc_html($cat87_hero_subtitle ? $cat87_hero_subtitle : 'Отводим дождевую и талую воду от дома, крыши, дорожек, парковки и участка.'); ?></p>
          ...
          <div class="hero__breadcramps"><a class="hero__home" href="<?php echo get_home_url(); ?>">Компания "Эксперты"
              | </a><span class="hero__active-page">Ливневая канализация</span></div>
        </div>
...
<section class="advantages wrapper">
    ...
    <section class="services wrapper">
      <h2 class="services__title">Услуги по ливневой канализации</h2>
      <div class="services__cards columns3">
        ...
        $posts = get_posts(array(
          ...
          'post_type' => 'post',
          'post_status' => 'publish',
          'post_name__in' => array('cena-livnevoy-kanalizatsii', 'montazh-livnevoy-kanalizatsii', 'livnevka-vokrug-doma', 'livnevka-na-uchastke', 'dozhdepriemniki-i-lotki', 'lineynyy-vodootvod', 'drenazh-i-livnevaya-kanalizatsiya', 'otvod-vody-s-kryshi', 'remont-livnevoy-kanalizatsii'),
        ...
        foreach ($posts as $post) {
          ...
          $livnevka_excerpt = get_the_excerpt();
          if (empty($livnevka_excerpt)) {
            $livnevka_excerpt = wp_trim_words(get_the_content(), 22);
          }
        ?>
        <div class="service" data-aos="fade-up" data-aos-duration="400">
          ...
          <div class="service__text-wrap">
            ...
            <p><?php echo esc_html($livnevka_excerpt); ?></p>
    ...
    <!-- 5. Blog placeholder -->
    <section class="services wrapper casesCustom">
      <h2 class="services__title">Полезное о ливневой канализации</h2>
      <div class="services__cards columns3">
        <?php
        ...
        ?>
          <p>Материалы раздела готовятся. Сейчас можно выбрать нужную услугу выше и запросить расчет по ливневой канализации.</p>
        <?php } ?>
      </div>
    </section>
    ...
    <section class="services wrapper portfolio">
      <h2 class="services__title"><?php echo esc_html($cat87_prices_title ? $cat87_prices_title : 'Стоимость ливневой канализации'); ?></h2>
      ...
      <div class="price-example">
        <h3><?php echo esc_html($cat87_estimate_title ? $cat87_estimate_title : 'Пример сметы на ливневую канализацию'); ?></h3>
        <ul>
        ...
        <a href="<?php echo esc_url($cat87_prices_link_url ? $cat87_prices_link_url : '/cena-livnevoy-kanalizatsii/'); ?>" class="btn--outline-custom"><?php echo esc_html($cat87_prices_link_text ? $cat87_prices_link_text : 'Подробные цены'); ?></a>
      </div>
    </section>
    ...
    <section class="services wrapper">
      <div class="seo-text" style="line-height: 1.6; margin-bottom: 40px;">
       <?php
       if ($cat91_region && !empty($cat91_region_seo_content)) {
         echo wp_kses_post($cat91_region_seo_content);
       } else {
         the_content();
       }
       ?>
    ...
    <section class="advantages wrapper">
      <div style="text-align: center; background: #f9f9f9; padding: 40px; border-radius: 10px;">
        <h2 class="cta-title"><?php echo esc_html($cat91_region ? 'Рассчитать ливневую канализацию в ' . $cat91_region['locative'] : 'Получите расчет ливневой канализации за 1 день'); ?></h2>
        ...
        <form class="cta-form form" id="calc" method="post" action="/server.php" style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
          <?php land76_render_form_security_fields('legacy-category-91-cta-v3'); ?>
