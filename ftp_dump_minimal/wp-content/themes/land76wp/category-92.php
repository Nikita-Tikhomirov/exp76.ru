<?php
/*
Template Name: Категория Автополив на участке
*/
?>
...
if (empty($cat87_price_rows) || !is_array($cat87_price_rows)) {
    $cat87_price_rows = array(
        array('service' => 'Монтаж автополива под ключ', 'price' => 'по расчету', 'term' => '2-7 дней'),
        array('service' => 'Автополив газона', 'price' => 'по расчету', 'term' => '1-5 дней'),
        array('service' => 'Капельный полив', 'price' => 'по расчету', 'term' => '1-3 дня'),
    );
}

if (empty($cat87_estimate_items) || !is_array($cat87_estimate_items)) {
    $cat87_estimate_items = array(
        array('item' => 'Схема зон полива - по расчету'),
        array('item' => 'Спринклеры, капельные линии и трубы - по расчету'),
        array('item' => 'Контроллер, клапаны и датчик дождя - по расчету'),
        array('item' => 'Монтаж, настройка и запуск - по расчету'),
    );
}
...
if (empty($cat87_faq_items) || !is_array($cat87_faq_items)) {
    $cat87_faq_items = array(
        array(
            'question' => 'Сколько стоит автополив на участке?',
            'answer' => 'Стоимость зависит от площади газона, количества зон, источника воды, насосного оборудования, контроллера, клапанов и типа полива. Точную цену считаем после осмотра и схемы.',
        ),
        array(
            'question' => 'Можно ли сделать автополив на готовом участке?',
            'answer' => 'Да, но схему нужно аккуратно привязать к газону, дорожкам, посадкам и существующим покрытиям, чтобы не испортить благоустройство.',
        ),
        array(
            'question' => 'Сколько времени занимает монтаж?',
            'answer' => 'Небольшой автополив можно смонтировать за несколько дней. Система на большой участок с несколькими зонами, насосом и автоматикой занимает дольше.',
        ),
    );
}
?>
    <!-- 1. Hero блок с полной структурой из header-page.php -->
      <section class="hero">
        ...
        <div class="hero__content wrapper">
          <h1 class="hero__title" data-aos="fade-right" data-aos-duration="800"><?php echo esc_html($cat87_hero_title ? $cat87_hero_title : 'Автополив на участке под ключ'); ?></h1>
          <p class="hero__subtitle" data-aos="fade-up" data-aos-duration="900"><?php echo esc_html($cat87_hero_subtitle ? $cat87_hero_subtitle : 'Проектируем и монтируем автоматический полив газона, посадок, клумб и сада с контроллером, клапанами и настройкой зон.'); ?></p>
          ...
          <div class="hero__breadcramps"><a class="hero__home" href="<?php echo get_home_url(); ?>">Компания "Эксперты"
              | </a><span class="hero__active-page">Автополив на участке</span></div>
        </div>
...
<section class="advantages wrapper">
    ...
    <section class="services wrapper">
      <h2 class="services__title">Услуги по автополиву на участке</h2>
      <div class="services__cards columns3">
        ...
        $posts = get_posts(array(
          ...
          'post_type' => 'post',
          'post_status' => 'publish',
          'post_name__in' => array('cena-avtopoliva-na-uchastke', 'montazh-avtopoliva', 'avtopoliv-gazona', 'kapelnyy-poliv', 'avtopoliv-sada', 'avtopoliv-teplitsy', 'proektirovanie-avtopoliva', 'nasos-i-emkost-dlya-poliva', 'obsluzhivanie-avtopoliva'),
        ...
        foreach ($posts as $post) {
          ...
          $autopoliv_excerpt = get_the_excerpt();
          if (empty($autopoliv_excerpt)) {
            $autopoliv_excerpt = wp_trim_words(get_the_content(), 22);
          }
        ?>
    ...
    <!-- 5. Blog placeholder -->
    <section class="services wrapper casesCustom">
      <h2 class="services__title">Полезное об автополиве на участке</h2>
      <div class="services__cards columns3">
        <?php
        ...
        ?>
          <p>Материалы раздела готовятся. Сейчас можно выбрать нужную услугу выше и запросить расчет автополива на участке.</p>
        <?php } ?>
      </div>
    </section>
    ...
    <section class="services wrapper portfolio">
      <h2 class="services__title"><?php echo esc_html($cat87_prices_title ? $cat87_prices_title : 'Стоимость автополива на участке'); ?></h2>
      ...
      <div class="price-example">
        <h3><?php echo esc_html($cat87_estimate_title ? $cat87_estimate_title : 'Пример сметы на автополив участка'); ?></h3>
        <ul>
        ...
        <a href="<?php echo esc_url($cat87_prices_link_url ? $cat87_prices_link_url : '/cena-avtopoliva-na-uchastke/'); ?>" class="btn--outline-custom"><?php echo esc_html($cat87_prices_link_text ? $cat87_prices_link_text : 'Подробные цены'); ?></a>
      </div>
    </section>
    ...
    <section class="advantages wrapper">
      <div style="text-align: center; background: #f9f9f9; padding: 40px; border-radius: 10px;">
        ...
        <form class="cta-form form" id="calc" method="post" action="/server.php" style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
          <?php land76_render_form_security_fields('legacy-category-92-cta-v3'); ?>
