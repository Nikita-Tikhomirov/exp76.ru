<?php
/*
Template Name: Категория Осушение участка
*/
?>
...
<?php
$cat90_region = function_exists('land76wp_get_current_osushenie_region') ? land76wp_get_current_osushenie_region() : null;
$cat87_term_context = $cat90_region ? 'category_90' : 'category_' . get_queried_object_id();
...
$cat90_region_seo_content = '';
if ($cat90_region) {
    $cat87_hero_title = 'Осушение участка в ' . $cat90_region['locative'] . ' под ключ';
    $cat87_hero_subtitle = $cat90_region['lead'];
    $cat87_hero_btn_primary_text = 'Рассчитать осушение';
    ...
    $cat87_prices_title = 'Стоимость осушения участка в ' . $cat90_region['locative'];
    $cat87_faq_title = 'Вопросы по осушению участка в ' . $cat90_region['locative'];
    $cat87_faq_items = array(
        array(
            'question' => 'Можно ли рассчитать осушение участка без выезда?',
            'answer' => 'Предварительно можно. Для точной схемы нужно увидеть рельеф, грунт, где стоит вода, куда ее можно отвести и какие постройки уже есть на участке.',
        ),
        array(
            'question' => 'Всегда ли осушение участка означает глубокий дренаж?',
            'answer' => 'Нет. Иногда достаточно поверхностного водоотвода, лотков, уклонов или локального дренажа. Мы подбираем решение под причину воды, а не ставим лишнюю систему.',
        ),
        array(
            'question' => 'Когда лучше делать осушение участка?',
            'answer' => 'Лучше до финишного благоустройства, дорожек, газона и посадок. Но на готовом участке тоже можно работать аккуратно, если правильно спланировать трассы и восстановление.',
        ),
    );
    $cat90_region_seo_content = !empty($cat90_region['content']) ? $cat90_region['content'] : '';
}
...
if (empty($cat87_price_rows) || !is_array($cat87_price_rows)) {
    $cat87_price_rows = array(
        array('service' => 'Осушение участка под ключ', 'price' => 'по расчету', 'term' => '3-10 дней'),
        array('service' => 'Дренаж для осушения участка', 'price' => 'от 2 800 ₽ за м.п.', 'term' => '3-7 дней'),
        array('service' => 'Поверхностный отвод воды', 'price' => 'от 2 200 ₽ за м.п.', 'term' => '1-5 дней'),
    );
}

if (empty($cat87_estimate_items) || !is_array($cat87_estimate_items)) {
    $cat87_estimate_items = array(
        array('item' => 'Диагностика причины воды - по расчету'),
        array('item' => 'Дренаж, лотки или водоотвод - по расчету'),
        array('item' => 'Колодцы и точка сброса воды - по расчету'),
        array('item' => 'Земляные работы и материалы - по расчету'),
    );
}
...
if (empty($cat87_faq_items) || !is_array($cat87_faq_items)) {
    $cat87_faq_items = array(
        array(
            'question' => 'Сколько стоит осушение участка?',
            'answer' => 'Стоимость зависит от причины воды, площади, грунта, рельефа, дренажа, водоотвода и точки сброса. Точную цену считаем после осмотра.',
        ),
        array(
            'question' => 'Всегда ли нужен дренаж?',
            'answer' => 'Нет. Дренаж нужен при воде в грунте, а для поверхностной воды иногда достаточно лотков, уклонов и водоотвода.',
        ),
        array(
            'question' => 'Сколько времени занимает монтаж?',
            'answer' => 'Срок зависит от площади и схемы. Локальный водоотвод занимает несколько дней, полноценная система осушения обычно дольше.',
        ),
    );
}
?>
    <!-- 1. Hero блок с полной структурой из header-page.php -->
      <section class="hero">
        ...
        <div class="hero__content wrapper">
          <h1 class="hero__title" data-aos="fade-right" data-aos-duration="800"><?php echo esc_html($cat87_hero_title ? $cat87_hero_title : 'Осушение участка под ключ'); ?></h1>
          <p class="hero__subtitle" data-aos="fade-up" data-aos-duration="900"><?php echo esc_html($cat87_hero_subtitle ? $cat87_hero_subtitle : 'Убираем лужи, заболачивание, мокрый грунт и проблемы с водой на участке.'); ?></p>
          ...
          <div class="hero__breadcramps"><a class="hero__home" href="<?php echo get_home_url(); ?>">Компания "Эксперты"
              | </a><span class="hero__active-page">Осушение участка</span></div>
        </div>
...
<section class="advantages wrapper">
    ...
    <section class="services wrapper">
      <h2 class="services__title">Услуги по осушению участка</h2>
      <div class="services__cards columns3">
        ...
        $posts = get_posts(array(
          ...
          'post_type' => 'post',
          'post_status' => 'publish',
          'post_name__in' => array('cena-osusheniya-uchastka', 'drenazh-dlya-osusheniya-uchastka', 'osushenie-pri-vysokih-gruntovyh-vodah', 'osushenie-zabolochennogo-uchastka', 'osushenie-dachnogo-uchastka', 'osushenie-glinistogo-uchastka', 'otvod-vody-s-uchastka', 'voda-posle-dozhdya-na-uchastke', 'proektirovanie-sistemy-osusheniya'),
        ...
        foreach ($posts as $post) {
          ...
          $osushenie_excerpt = get_the_excerpt();
          if (empty($osushenie_excerpt)) {
            $osushenie_excerpt = wp_trim_words(get_the_content(), 22);
          }
        ?>
        <div class="service" data-aos="fade-up" data-aos-duration="400">
          ...
          <div class="service__text-wrap">
            ...
            <p><?php echo esc_html($osushenie_excerpt); ?></p>
    ...
    <!-- 5. Blog placeholder -->
    <section class="services wrapper casesCustom">
      <h2 class="services__title">Полезное об осушении участка</h2>
      <div class="services__cards columns3">
        <?php
        ...
        ?>
          <p>Материалы раздела готовятся. Сейчас можно выбрать нужную услугу выше и запросить расчет по осушению участка.</p>
        <?php } ?>
      </div>
    </section>
    ...
    <section class="services wrapper portfolio">
      <h2 class="services__title"><?php echo esc_html($cat87_prices_title ? $cat87_prices_title : 'Стоимость осушения участка'); ?></h2>
      ...
      <div class="price-example">
        <h3><?php echo esc_html($cat87_estimate_title ? $cat87_estimate_title : 'Пример сметы на осушение участка'); ?></h3>
        <ul>
        ...
        <a href="<?php echo esc_url($cat87_prices_link_url ? $cat87_prices_link_url : '/cena-osusheniya-uchastka/'); ?>" class="btn--outline-custom"><?php echo esc_html($cat87_prices_link_text ? $cat87_prices_link_text : 'Подробные цены'); ?></a>
      </div>
    </section>
    ...
    <section class="services wrapper">
      <div class="seo-text" style="line-height: 1.6; margin-bottom: 40px;">
       <?php
       if ($cat90_region && !empty($cat90_region_seo_content)) {
         echo wp_kses_post($cat90_region_seo_content);
       } else {
         the_content();
       }
       ?>
    ...
    <section class="advantages wrapper">
      <div style="text-align: center; background: #f9f9f9; padding: 40px; border-radius: 10px;">
        <h2 class="cta-title"><?php echo esc_html($cat90_region ? 'Рассчитать осушение участка в ' . $cat90_region['locative'] : 'Получите расчет осушения участка за 1 день'); ?></h2>
        ...
        <form class="cta-form form" id="calc" method="post" action="/server.php" style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
          <?php land76_render_form_security_fields('legacy-category-90-cta-v3'); ?>
