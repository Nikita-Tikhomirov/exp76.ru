...
function land76wp_autopoliv_import_default_json_path()
{
    return trailingslashit(get_template_directory()) . 'import/autopoliv-import.json';
}

function land76wp_autopoliv_import_update_acf_fields(array $fields, $context, array &$stats)
{
...
function land76wp_autopoliv_import_upsert_post(array $post_payload, array &$stats)
{
    ...
    land76wp_autopoliv_import_update_acf_fields($acf_fields, $post_id, $stats);
}

function land76wp_autopoliv_import_cleanup_stale_posts(array $cleanup_payload, array &$stats)
{
    $category_id = !empty($cleanup_payload['category_id']) ? (int) $cleanup_payload['category_id'] : 92;
...
function land76wp_run_autopoliv_import($json_path = '')
{
    $stats = array(
        'json_path' => '',
    ...
    $json_path = $json_path ? $json_path : land76wp_autopoliv_import_default_json_path();
    ...
    if (!empty($payload['category']) && is_array($payload['category'])) {
        $term_id = !empty($payload['category']['term_id']) ? (int) $payload['category']['term_id'] : 92;
        $term_context = 'category_' . $term_id;
        $term_fields = !empty($payload['category']['acf']) && is_array($payload['category']['acf'])
            ? $payload['category']['acf']
            : array();

        land76wp_autopoliv_import_update_acf_fields($term_fields, $term_context, $stats);
        $stats['category_updated'] = true;
    }

    if (!empty($payload['cleanup']) && is_array($payload['cleanup'])) {
        land76wp_autopoliv_import_cleanup_stale_posts($payload['cleanup'], $stats);
    }
    ...
    if (!empty($payload['posts']) && is_array($payload['posts'])) {
        foreach ($payload['posts'] as $post_payload) {
            ...
            land76wp_autopoliv_import_upsert_post($post_payload, $stats);
        }
    }
...
function land76wp_autopoliv_import_maybe_run_from_admin()
{
    if (!is_admin() || !current_user_can('manage_options')) {
        return;
    }

    if (empty($_GET['land76_run_autopoliv_import'])) {
        return;
    }

    $result = land76wp_run_autopoliv_import();

    wp_die(
        '<h1>autopoliv import result</h1><pre>' .
        esc_html(wp_json_encode($result, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) .
        '</pre>'
    );
}
add_action('admin_init', 'land76wp_autopoliv_import_maybe_run_from_admin');
