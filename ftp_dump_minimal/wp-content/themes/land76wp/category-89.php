<?php
/*
Template Name: Категория Укладка тротуарной плитки
*/
?>
...
if (empty($cat87_price_rows) || !is_array($cat87_price_rows)) {
    $cat87_price_rows = array(
        array('service' => 'Укладка тротуарной плитки под ключ', 'price' => 'от 2 800 ₽ за м²', 'term' => '3-7 дней'),
        array('service' => 'Подготовка основания под плитку', 'price' => 'от 1 500 ₽ за м²', 'term' => '1-3 дня'),
        array('service' => 'Площадка под автомобиль', 'price' => 'от 3 800 ₽ за м²', 'term' => '4-8 дней'),
    );
}

if (empty($cat87_estimate_items) || !is_array($cat87_estimate_items)) {
    $cat87_estimate_items = array(
        array('item' => 'Разметка и подготовка основания - по расчету'),
        array('item' => 'Песчано-щебеночная подушка и трамбовка - по расчету'),
        array('item' => 'Бордюры, уклоны и водоотвод - по расчету'),
        array('item' => 'Укладка плитки и заполнение швов - по расчету'),
    );
}
...
if (empty($cat87_faq_items) || !is_array($cat87_faq_items)) {
    $cat87_faq_items = array(
        array(
            'question' => 'Сколько стоит укладка тротуарной плитки?',
            'answer' => 'Стоимость зависит от площади, основания, плитки, бордюров, водоотвода и нагрузки на покрытие. Точную цену считаем после замеров.',
        ),
        array(
            'question' => 'Можно ли уложить плитку на старое основание?',
            'answer' => 'Можно только если основание стабильное и держит уклон. При просадках покрытие лучше разобрать и пересобрать основание.',
        ),
        array(
            'question' => 'Сколько времени занимает монтаж?',
            'answer' => 'Небольшие дорожки обычно занимают несколько дней, двор или парковка требуют больше времени из-за подготовки основания и бордюров.',
        ),
    );
}
?>
    <!-- 1. Hero блок с полной структурой из header-page.php -->
      <section class="hero">
        ...
        <div class="hero__content wrapper">
          <h1 class="hero__title" data-aos="fade-right" data-aos-duration="800"><?php echo esc_html($cat87_hero_title ? $cat87_hero_title : 'Укладка тротуарной плитки под ключ'); ?></h1>
          <p class="hero__subtitle" data-aos="fade-up" data-aos-duration="900"><?php echo esc_html($cat87_hero_subtitle ? $cat87_hero_subtitle : 'Делаем дорожки, двор, парковку и площадки с правильным основанием, уклоном и водоотводом.'); ?></p>
...
<section class="advantages wrapper">
    ...
    <section class="services wrapper">
      <h2 class="services__title">Услуги по укладке тротуарной плитки</h2>
      <div class="services__cards columns3">
        ...
        $posts = get_posts(array(
          ...
          'post_type' => 'post',
          'post_status' => 'publish',
          'post_name__in' => array('cena-ukladki-trotuarnoy-plitki', 'podgotovka-osnovaniya-pod-plitku', 'sadovye-dorozhki-iz-plitki', 'ploshchadka-pod-avto-iz-plitki', 'dvor-iz-trotuarnoy-plitki', 'ukladka-bruschatki', 'otmostka-iz-trotuarnoy-plitki', 'bordyury-i-vodootvod-dlya-plitki', 'remont-trotuarnoy-plitki'),
        ...
        foreach ($posts as $post) {
          ...
          $plitka_excerpt = get_the_excerpt();
          if (empty($plitka_excerpt)) {
            $plitka_excerpt = wp_trim_words(get_the_content(), 22);
          }
        ?>
    ...
    <!-- 5. Blog placeholder -->
    <section class="services wrapper casesCustom">
      <h2 class="services__title">Полезное об укладке тротуарной плитки</h2>
      <div class="services__cards columns3">
        <?php
        ...
        ?>
          <p>Материалы раздела готовятся. Сейчас можно выбрать нужную услугу выше и запросить расчет по укладке плитки.</p>
        <?php } ?>
      </div>
    </section>
    ...
    <section class="services wrapper portfolio">
      <h2 class="services__title"><?php echo esc_html($cat87_prices_title ? $cat87_prices_title : 'Стоимость укладки тротуарной плитки'); ?></h2>
      ...
      <div class="price-example">
        <h3><?php echo esc_html($cat87_estimate_title ? $cat87_estimate_title : 'Пример сметы на укладку тротуарной плитки'); ?></h3>
        <ul>
        ...
        <a href="<?php echo esc_url($cat87_prices_link_url ? $cat87_prices_link_url : '/cena-ukladki-trotuarnoy-plitki/'); ?>" class="btn--outline-custom"><?php echo esc_html($cat87_prices_link_text ? $cat87_prices_link_text : 'Подробные цены'); ?></a>
      </div>
    </section>
